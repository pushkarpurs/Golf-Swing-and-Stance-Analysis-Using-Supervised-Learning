...
export default function BackView() {
	...
	useEffect(() => {
		fetch('http://localhost:5001/start_feed')
        ...
        return () => {
            ...
            fetch('http://localhost:5001/stop_feed')
                .then(response => response.json())
                .then(data => {
    ...
    return (
        <div className="lg:px-20 px-8 md:text-base text-xs lg:text-base">
			...
			style={{ fontFamily: 'Trebuchet MS, Sans-serif' }}>Back View</h1>
			<div className="video-container border-4 border-gray-400 rounded-md p-2">
                {isStreaming ? (
					<>
                    <img src={`http://localhost:5001/get_feed?${Date.now()}`} alt="Feed From Py Back End" className="mx-auto" />
